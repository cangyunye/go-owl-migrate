...
export async function render(root /*Element*/, params) {
    root.innerHTML = ''
        ...
        +   '<div class="field">'
        +     '<label>范围</label>'
        +     '<input type="text" id="em-scope" class="mono" value="all" placeholder="all / schema:NAME / table:GLOB[,GLOB] / schema:NAME:table:GLOB[,GLOB]">'
        +     '<div class="field-help"><code>all</code> = 整个 schema · <code>schema:NAME</code> = 指定 schema（例 <code>schema:SCOTT</code>）· <code>table:GLOB[,GLOB]</code> = 当前 schema 内按表名 glob 选取（例 <code>table:EMP,DEPT*</code>）· <code>schema:NAME:table:GLOB[,GLOB]</code> = 指定 schema 内按表名 glob 选取（例 <code>schema:SCOTT:table:EMP,DEPT*</code>）；glob 大小写不敏感，支持 <code>*</code> <code>?</code> <code>[]</code>；不支持的对象类型由后端按源库能力校验报错</div>'
        +   '</div>'
        +   '<div class="field">'
        +     '<label>对象类型</label>'
