...
| GET /api/v1/config/status | Reports the config path, on-disk status, dialect/type, metadata-loaded state, and the password-free `source`/`target` endpoint identities. Each identity carries `type`, `schema`, and — parsed from the real DSN server-side, since the browser only holds masked values — `user`, `host`, `port`, `database`, `label` ("user@host:port/database", or the file path for embedded databases) plus `ref` when the DSN is a `datasource:<name>` reference. Passwords never appear. `metadata_stale` is true when the loaded tables came from a different source than the config now specifies (schema/DSN/type/metadata path changed): the migrate page then re-extracts automatically, and generation endpoints refuse the stale model. A password-only change, a differently-written but equivalent DSN, a data-source reference to the same endpoint, or a config section that is simply omitted do **not** count as stale. |
...
| POST /api/v1/metadata/row-count | Exact `COUNT(*)` for the requested source tables. Body `{"tables":[{"schema":"public","name":"emp"}]}`; streams `application/x-ndjson`, one `{"schema","name","rows"}` per table **in request order** (a table that fails, or that is not in the loaded metadata, reports `{"error":"…"}` on its own line and does not stop the run), then a `{"done":true,"counted":N,"failed":M}` summary. Counts go to the caller only; the loaded metadata keeps its planner estimate. Needs loaded metadata plus a configured source; ≤1000 tables per request, 2 minutes per table (`source.query_timeout` overrides). The migrate view's table picker drives it: click a table to count it, or 「全量统计」 to walk the visible list top-to-bottom. |
...
| POST /api/v1/migrate/preflight | Dry-run checks (config, metadata extractable, table selection, target ping; `?mode=sql-out` skips the target). Never writes or spawns a worker. |
