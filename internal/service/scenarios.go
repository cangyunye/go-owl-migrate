...
func DSNExamples() map[string]string {
	return map[string]string{
		...
		"oceanbase-mysql":  "user@tenant:pass@tcp(host:2881)/dbname (MySQL mode; 用户名须带租户, 如 root@test; 2883 走 OBProxy)",
		...
		"oceanbase":        "user@tenant:pass@tcp(host:2881)/dbname (OceanBase MySQL mode; 用户名须带租户如 root@test; Oracle tenants: use type oceanbase-oracle)",
